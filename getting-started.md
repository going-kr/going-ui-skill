...
> `project-brief.md` 참조. 인터뷰 경로 A/B/C 중 택 1.
...
> `gud-structure.md` + `controls/*.md` 참조. 브리프 기반으로 Claude가 JSON 생성.
...
> `code-pattern.md`, `comm-pattern.md` 참조.
...
public class Main
{
    ...
    public static DeviceManager DevMgr { get; set; } = new DeviceManager();
    public static DataManager DataMgr { get; set; } = new DataManager();
}
```
...
### 4-3. DeviceData + DeviceManager (Modbus TCP)

> 1대의 슬레이브만 사용하더라도 **DeviceData 패턴**을 적용한다.
> DeviceData로 레지스터 의미를 래핑하면 Page 코드가 간결해지고, 슬레이브 추가 시 확장이 용이하다.
...
namespace MotorControl
{
    // DeviceData: 슬레이브 1대의 레지스터 의미를 래핑
    public class MotorData
    {
        public int ID { get; set; }             // 슬레이브 번호
        private MasterTCP Master { get; set; }

        public MotorData(MasterTCP master, int slaveId)
        {
            Master = master;
            ID = slaveId;
        }

        // 읽기 — GetWord/GetBit로 래핑
        public bool IsRunning   => Master.GetWord(ID, "D0") == 1;   // RunState
        public int  CurrentRPM  => Master.GetWord(ID, "D1") ?? 0;   // CurrentRPM
        public int  ErrorCode   => Master.GetWord(ID, "D2") ?? 0;   // ErrorCode
        public int  TargetSpeed => Master.GetWord(ID, "D16") ?? 0;  // 0x0010 = 16

        // 쓰기 — Set{기능이름}()로 래핑
        public void SetTargetSpeed(int rpm) => Master.SetWord(ID, "D16", rpm);
        public void SetRunCmd(bool on)      => Master.SetBit(ID, "P0", on);
        public void SetStopCmd(bool on)     => Master.SetBit(ID, "P1", on);
    }

    // DeviceManager: MasterTCP + DeviceData 관리
    public class DeviceManager
    {
        public MasterTCP TCP { get; } = new MasterTCP();
        public MotorData Data { get; private set; }
        ...
        public DeviceManager()
        {
            ...
            Data = new MotorData(TCP, 1);     // 슬레이브 1번
        }
        ...
        public void Start()
        {
            ...
            TCP.RemoteIP   = setting.IP;
            TCP.RemotePort = setting.Port;
...
namespace MotorControl.Pages
{
    public partial class PageMain : GoPage
    {
        public PageMain()
        {
            ...
            {
                Main.DevMgr.Data.SetRunCmd(true);
            };

            // 정지 버튼
            btnStop.ButtonClicked += (s, e) =>
            {
                Main.DevMgr.Data.SetStopCmd(true);
            };

            // 속도 슬라이더 변경
            sldSpeed.ValueChanged += (s, e) =>
            {
                Main.DevMgr.Data.SetTargetSpeed((int)sldSpeed.Value);
            };

            // 설정 페이지로 이동 (설정 버튼이 있다면)
            // btnSetting.ButtonClicked += (s, e) =>
            // {
            //     Design?.SetPage("PageSetting");
            // };
        }
        ...
        protected override void OnUpdate()
        {
            ...
            lampRun.OnOff = data.IsRunning;           // 운전 램프
            lampStop.OnOff = !data.IsRunning;          // 정지 램프
            valRPM.Value = data.CurrentRPM;            // RPM 표시
            sldSpeed.Value = data.TargetSpeed;         // 슬라이더 현재값 동기화
            lblStatus.Text = Main.DevMgr.IsConnected
                ? (data.IsRunning ? "운전 중" : "정지")
                : "통신 끊김";
...
| 문서 | 용도 |
|------|------|
| `project-brief.md` | 인터뷰 절차 상세 |
| `gud-structure.md` | .gud 구조 상세 |
| `code-pattern.md` | C# 패턴 상세 |
| `comm-pattern.md` | 통신 패턴 상세 |
| `deploy-cli.md` | 장치 배포 |
