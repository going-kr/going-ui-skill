...
> **이 문서의 역할**: 통신 코드 작성 시 "어떤 패턴으로, 어떤 순서로" 작성하는지 안내.
> 완성된 코드 예제 + 자주 하는 실수 방지가 핵심.
>
> **comm/*.md의 역할**: 각 클래스의 속성/메서드/이벤트 시그니처 레퍼런스.
> "이 메서드의 파라미터가 뭐지?" 할 때 참조.
>
> **사용 흐름**: comm-pattern.md로 패턴을 잡고 → 시그니처 확인이 필요하면 comm/*.md 참조

## ⚠ 자주 하는 실수 19개

코드 생성 전 반드시 확인. 아래 표의 "잘못" 패턴이 출력에 포함되면 즉시 수정할 것.

| # | ❌ 잘못 | ✅ 올바른 | 설명 |
|---|---------|----------|------|
| 1 | `ModbusRTUMaster`에서 `WordAreas` 사용 | `MasterRTU`에서 `WordAreas` 사용 | WordAreas는 래퍼에만 있음 |
| 2 | `MasterRTU`에서 `AutoWordRead_FC3` 호출 | `RTU.MonitorWord_F3(slave, addr, len)` | 래퍼는 Monitor 메서드 사용 |
| 3 | `MasterTCP.Host = "..."` | `MasterTCP.RemoteIP = "..."` | MasterTCP 프로퍼티명 주의 |
| 4 | `MasterTCP.Port = 502` | `MasterTCP.RemotePort = 502` | MasterTCP 프로퍼티명 주의 |
| 5 | `s.SlaveAddress` | `s.Slave` | 이벤트 인자 프로퍼티명 |
| 6 | `s.Message` (MQTT) | `s.Datas` (byte[]) | MQTT 수신은 byte[], UTF-8 디코딩 필요 |
| 7 | `MQQosLevel.AtLeastOnce` | `MQQos.LeastOnce` | 열거형 이름과 값 모두 다름 |
| 8 | `mqtt.Port = 1883` | (설정 불가) | MQClient에 Port 프로퍼티 없음 |
| 9 | `ManualWordWrite_FC16(...)` | `ManualMultiWordWrite_FC16(...)` | FC16은 Multi 접두사 필요 |
| 10 | `ManualBitWrite_FC15(...)` | `ManualMultiBitWrite_FC15(...)` | FC15는 Multi 접두사 필요 |
| 11 | `GetWord("D", 0)` | `GetWord(1, "D0")` | 첫 인자=slave번호, 둘째="영역+오프셋" 문자열 |
| 12 | `SlaveTCP.Port = 502` | `SlaveTCP.LocalPort = 502` | SlaveTCP는 `LocalPort` 사용 |
| 13 | `SlaveTCP.DeviceOpened` | `SlaveTCP.SocketConnected` | TCP 슬레이브는 Socket 이벤트 사용 |
| 14 | `wordMem[0] = 100` | `wordMem[0].W = 100` | WordMemory는 `.W` 프로퍼티 필요 (WordRef 반환) |
| 15 | `bitMem[0].W` | `bitMem[0]` | BitMemory는 bool 직접 반환 (.W 없음) |
| 16 | `wordMem[0].Value` | `wordMem[0].W` | 구 API의 `.Value`는 `.W`(ushort)로 변경됨 |
| 17 | 일반 폴링에 `ModbusRTUMaster` 사용 | `MasterRTU` 사용 | 저수준은 특수 목적 전용 |
| 18 | DeviceData에 `ID`(slaveId) 없음 | DeviceData에 항상 `ID` 프로퍼티 포함 | DeviceData는 반드시 slaveId를 가짐 |
| 19 | `MasterRTU`에서 통신 상태를 `CommState`로 판단 | `RTU.IsOpen` 또는 `DeviceOpened/Closed` 이벤트 | MasterRTU는 IsOpen으로 판단 |
...
> ⚠ **슬레이브가 1대뿐이더라도 반드시 DeviceData 패턴을 사용하라.**
> DeviceManager에서 직접 GetWord/GetBit를 호출하지 말 것.
> 레지스터 주소 ↔ 속성명 매핑은 DeviceData에 집중시켜야 주소 변경 시 한 곳만 수정하면 된다.
> DeviceData 상세는 아래 "DeviceData 모델 패턴" 섹션 참조.

```csharp
// Datas/DeviceData.cs — 슬레이브별 데이터 모델 (1대라도 반드시 작성)
public class DeviceData
{
    ...
    // 통신 상태 — 슬레이브별 개별 판정 (IsOpen은 포트 전체 상태만)
...
```csharp
// Managers/DeviceManager.cs — MasterRTU + DeviceData 조합
public class DeviceManager
{
    ...
    public DeviceManager()
    {
        ...
        Data = new DeviceData(RTU, 1);  // 1대라도 DeviceData 사용
    ...
    public void Start()
    {
        ...
        // ⚠ 쓰기 대상 레지스터도 반드시 Monitor로 읽기 등록할 것
...
```csharp
// Pages/PageMain.cs — DeviceData를 통해 읽기/쓰기
protected override void OnUpdate()
{
    // 읽기 — DeviceData 프로퍼티로 접근
    var data = Main.DevMgr.Data;
    valTemp.Value    = data.Temperature;
    valPressure.Text = $"{data.Pressure} kPa";
    lampRun.OnOff    = data.RunState;
    lampComm.OnOff   = data.IsAlive();
}

// 이벤트 바인딩 (InitializeComponent 이후)
btnStart.ButtonClicked += (s, e) =>
{
    // 쓰기 — DeviceData의 Set 메서드로 접근
    Main.DevMgr.Data.SetRun(true);
};

btnStop.ButtonClicked += (s, e) =>
{
    Main.DevMgr.Data.SetRun(false);
};

sldTemp.SliderDragCompleted += (s, e) =>
{
    Main.DevMgr.Data.SetTargetTemp(sldTemp.Value);
};
```
...
SlaveRTU와 동일한 구조. TCP 서버(리스너) 역할. 프로퍼티/이벤트가 다르므로 주의.

> **SlaveRTU와의 핵심 차이**:
> - 프로퍼티: `Port`/`Baudrate` 대신 `LocalPort` 사용
> - 이벤트: `DeviceOpened`/`DeviceClosed` 대신 `SocketConnected`/`SocketDisconnected` 사용
> - `IsOpen` 프로퍼티 없음 (TCP 소켓 기반이므로)
...
public class DeviceManager
{
    ...
    public DeviceManager()
    {
        // 메모리 영역 등록 — SlaveRTU와 동일한 WordMemory/BitMemory 사용
        Slave.WordAreas.Add(0x0000, new WordMemory(100));  // 워드 100개
        Slave.BitAreas.Add(0x0000, new BitMemory(32));     // 비트 32개

        // ⚠ TCP는 Socket 이벤트 사용 (DeviceOpened/Closed 아님!)
    ...
    // 메모리 읽기/쓰기 — SlaveRTU와 동일한 인덱서 패턴
    public int ReadWord(int baseAddr, int offset)
    {
        var mem = Slave.WordAreas[baseAddr];
        return mem[offset].W;                // WordMemory: mem[idx].W (ushort)
    ...
    // 더블워드/실수 읽기·쓰기 예시
    public uint ReadDWord(int baseAddr, int offset)
    {
        var mem = Slave.WordAreas[baseAddr];
        return mem[offset].DW;               // 더블워드 (uint, 2워드 사용)
    }

    public void WriteDWord(int baseAddr, int offset, uint value)
    {
        var mem = Slave.WordAreas[baseAddr];
        mem[offset].DW = value;
    }

    public float ReadFloat(int baseAddr, int offset)
    {
        var mem = Slave.WordAreas[baseAddr];
        return mem[offset].R;                // 실수 (float, 2워드 사용)
    }

    public void WriteFloat(int baseAddr, int offset, float value)
    {
        var mem = Slave.WordAreas[baseAddr];
        mem[offset].R = value;
    }
...
| 프로퍼티 | 타입 | 설명 |
|---------|------|------|
| `Slave` | int (기본 1) | 슬레이브 번호. 마스터 요청의 Unit ID와 일치해야 함 |
| `LocalPort` | int | 리슨 포트 (기본 502). **`Port` 아님 — 주의!** |
| `IsStart` | bool (읽기전용) | 시작 상태 |
| `BitAreas` | Dictionary\<int, BitMemory\> | 비트 메모리 영역 (시작주소 → BitMemory) |
| `WordAreas` | Dictionary\<int, WordMemory\> | 워드 메모리 영역 (시작주소 → WordMemory) |
| `Tag` | object? | 사용자 데이터 |

> **SlaveTCP에는 `IsOpen` 프로퍼티 없음** — TCP는 소켓 기반이므로 포트 열림 개념이 없다. 연결 상태는 `SocketConnected`/`SocketDisconnected` 이벤트로 판단.
...
### SlaveTCP vs SlaveRTU 차이 요약

| 항목 | SlaveRTU | SlaveTCP |
|------|----------|----------|
| 통신 설정 | `Port`, `Baudrate`, `Parity`, `DataBits`, `StopBits` | `LocalPort` |
| 연결 이벤트 | `DeviceOpened` / `DeviceClosed` | `SocketConnected` / `SocketDisconnected` |
| 상태 프로퍼티 | `IsOpen` 있음 | `IsOpen` **없음** |
| 메모리 접근 | WordMemory/BitMemory 동일 | WordMemory/BitMemory 동일 |
