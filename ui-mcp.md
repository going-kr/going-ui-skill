...
gtcli는 self-contained 바이너리 (약 15MB). .NET 런타임 없이 실행 가능.

**방법 1 — GitHub Release 다운로드 (권장)**:
```bash
# 최신 버전 다운로드
gh release download -R going-kr/going-ui-skill -p "gtcli*" -D .
chmod +x gtcli  # Linux/macOS
```

**방법 2 — 스킬 디렉터리에 직접 배치**:
`gtcli.exe`를 스킬 디렉터리에 복사하면 상대경로로 실행 가능.

### 실행 예시

```bash
"./gtcli.exe" scan
"./gtcli.exe" status 172.30.1.13 --token "토큰값"
```
